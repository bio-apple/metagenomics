...
parser=argparse.ArgumentParser("MetaPhlAn4 uses a database of clade-specific marker genes to classify\n")
...
cmd="docker run -v %s:/raw_data/ -v %s:/ref/ -v %s:/outdir/ %s "%(in_dir,reference_dir,args.outdir,docker_name)
bowtie2_index=""
for i in os.listdir(reference_dir):
    if i.endswith(".pkl"):
        bowtie2_index=i.split('.pkl')[0]

cmd+="sh -c \"export PATH=/opt/conda/envs/metaphlan/bin/:$PATH && metaphlan /raw_data/%s,/raw_data/%s " \
     "--force --nproc 48 --bowtie2db /ref/ --add_viruses " \
